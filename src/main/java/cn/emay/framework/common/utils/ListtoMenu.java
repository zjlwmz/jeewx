...
import cn.emay.framework.core.enums.SysACEIconEnum;
 ...
 * 
 * @author 张代浩
 *  update-begin--Author:jg_longjb龙金波  Date:20150313 for：本文件中所有.getFunctions().size()替换为.getSubFunctionSize();
...
public class ListtoMenu {
	 ...
	 * @return
	 */
	public static String getBootMenu(List<Function> pFunctions,
	 ...
	 * @return
	 */
	public static String getEasyuiMenu(List<Function> pFunctions,
			List<Function> functions) {
		StringBuffer menuString = new StringBuffer();
		for (Function pFunction : pFunctions) {
			menuString.append("<div  title=\"" + getMutiLang(pFunction.getFunctionName())
					+ "\" iconCls=\"" + ResourceUtil.allTSIcons.get(pFunction.getIcon().getId()).getIconClas()
					+ "\">");
			...
			for (Function function : functions) {
				if (function.getParentFunction().getId().equals(pFunction.getId())) {
					...
					if (function.getIcon() != null) {
						icon = ResourceUtil.allTSIcons.get(function.getIcon().getId()).getIconClas();
					}
	...
	public static String getEasyuiMultistageMenu(
			Map<Integer, List<Function>> map) {
		StringBuffer menuString = new StringBuffer();
		List<Function> list = map.get(0);
		for (Function function : list) {
			menuString.append("<div   title=\"" + getMutiLang(function.getFunctionName())
					+ "\" iconCls=\"" + ResourceUtil.allTSIcons.get(function.getIcon().getId()).getIconClas()
					+ "\">");
     ...
     * @param map  the map of Map<Integer, List<Function>>
     * @param style 样式：easyui-经典风格、shortcut-shortcut风格
     * @return
     */
	public static String getEasyuiMultistageTree(Map<Integer, List<Function>> map, String style) {
	 ...
	 * @return
	 */
	private static String getChild(Function parent,int level,Map<Integer, List<Function>> map){
		StringBuffer menuString = new StringBuffer();
	 ...
	 * @return
	 */
	private static String getChildOfTree(Function parent,int level,Map<Integer, List<Function>> map){
		StringBuffer menuString = new StringBuffer();
		List<Function> list = map.get(level);
		for (Function function : list) {
			if (function.getParentFunction().getId().equals(parent.getId())){
				if(!function.hasSubFunction(map)){
					...
					menuString.append("<li state=\"closed\" iconCls=\"" + ResourceUtil.allTSIcons.get(function.getIcon().getId()).getIconClas()+"\" ><span>"+ getMutiLang(function.getFunctionName()) +"</span>");
					menuString.append("<ul >");
	 ...
	 * @return
	 */
	private static String getLeaf(Function function){
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getIcon() != null) {
			icon = ResourceUtil.allTSIcons.get(function.getIcon().getId()).getIconClas();
	 ...
	 * @return
	 */
	private static String getLeafOfTree(Function function){
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getIcon() != null) {
			icon = ResourceUtil.allTSIcons.get(function.getIcon().getId()).getIconClas();
	 ...
	 * @return
	 */
	public static String getBootstrapMenu(Map<Integer, List<Function>> map) {
		StringBuffer menuString = new StringBuffer();
		...
		for (Function pFunction : pFunctions) {
			...
			menuString.append("			<span class=\"bootstrap-icon\" style=\"background-image: url('"+ResourceUtil.allTSIcons.get(pFunction.getIcon().getId()).getIconPath()+"')\"></span> "+pFunction.getFunctionName()+" ");
			if(hasSub){
	...
	* @throws
	 */
	private static String getBootStrapChild(Function parent,int level,Map<Integer, List<Function>> map){
		StringBuffer menuString = new StringBuffer();
		List<Function> list = map.get(level);
...
//		System.out.println(menu+"$$"+desk+"$$"+data+"$$"+"{\"total\":"+n+"}");
	 ...
	 * @throws
	 */
	private static String getWebosChild(Function parent,int level,Map<Integer, List<Function>> map){
		StringBuffer menuString = new StringBuffer();
		String menu = "";
		List<Function> list = map.get(level);
		if(list==null || list.size()==0){
		...
		for (Function function : list) {
			if (function.getParentFunction().getId().equals(parent.getId())){
				boolean hasSub = function.hasSubFunction(map);
				menuString.append("\""+ function.getId() + "\":");
				menuString.append("{\"id\":\""+function.getId()+"\",\"name\":\""+ getMutiLang(function.getFunctionName()) +"\",\"path\":\""+function.getIcon().getIconPath()+"\",\"url\":\""+function.getFunctionUrl()+"\",\"level\":\""+function.getFunctionLevel()+"\"}");
...
//		data = dataString.substring(0, dataString.toString().length()-1);
	...
	* @throws
	...
	public static String getDIYMultistageTree(Map<Integer, List<Function>> map) {
		if(map==null||map.size()==0||!map.containsKey(0)){return "不具有任何权限,\n请找管理员分配权限";}
		StringBuffer menuString = new StringBuffer();
		List<Function> list = map.get(0);
        int curIndex = 0;
	...
	private static String getDIYSubMenu(Function parent,int level,Map<Integer, List<Function>> map){
		StringBuffer menuString = new StringBuffer();
		List<Function> list = map.get(level);
		for (Function function : list) {
			if (function.getParentFunction().getId().equals(parent.getId())){
	...
	public static String getAceMultistageTree(Map<Integer, List<Function>> map) {
		if(map==null||map.size()==0||!map.containsKey(0)){return "不具有任何权限,\n请找管理员分配权限";}
		StringBuffer menuString = new StringBuffer();
		List<Function> list = map.get(0);
        int curIndex = 0;
            for (Function function : list) {
                menuString.append("<li>");
                menuString.append("<a href=\"#\" class=\"dropdown-toggle\" ><i class=\""+SysACEIconEnum.toEnum(function.getIcon().getIconClas()).getThemes()+"\"></i>");
	...
	private static String getACESubMenu(Function parent,int level,Map<Integer, List<Function>> map){
		StringBuffer menuString = new StringBuffer();
		List<Function> list = map.get(level);
		for (Function function : list) {
			if (function.getParentFunction().getId().equals(parent.getId())){
	...
	private static String getLeafOfACETree(Function function){
		StringBuffer menuString = new StringBuffer();
//		String icon = "folder";
//		if (function.getIcon() != null) {
//			icon = ResourceUtil.allTSIcons.get(function.getIcon().getId()).getIconClas();
...
//		menuString.append(icon);
	...
	private static String getLeafOfDIYTree(Function function){
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getIcon() != null) {
			icon = ResourceUtil.allTSIcons.get(function.getIcon().getId()).getIconClas();
	...
	public static String getHplusMultistageTree(Map<Integer, List<Function>> map) {
		if(map==null||map.size()==0||!map.containsKey(0)){return "不具有任何权限,\n请找管理员分配权限";}
		StringBuffer menuString = new StringBuffer();
		List<Function> list = map.get(0);
		int curIndex = 0;
	...
	private static String getHplusSubMenu(Function parent, int level, Map<Integer, List<Function>> map) {
		StringBuffer menuString = new StringBuffer();
		List<Function> list = map.get(level);
		for (Function function : list) {
			if (function.getParentFunction().getId().equals(parent.getId())){
	...
	private static String getLeafOfHplusTree(Function function) {
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getIcon() != null) {
			icon = ResourceUtil.allTSIcons.get(function.getIcon().getId()).getIconClas();
