...
import cn.emay.framework.core.constant.DataBaseConstant;
...
public class ResourceUtil {
	 ...
	 * 缓存系统图标【缓存】
	 */
	public static Map<String, Icon> allTSIcons = new HashMap<String, Icon>();

	private static final ResourceBundle bundle = java.util.ResourceBundle.getBundle("jeewx");

	/**
	 * 属性文件[resources/jeewx.properties] #默认开启模糊查询方式 1为开启 条件无需带*就能模糊查询[暂时取消]
	 * fuzzySearch=0
	 */
...
//	}
	...
	public static String getUserSystemData(String key) {
		...
		if (key.equals(DataBaseConstant.SYS_COMPANY_CODE) || key.equals(DataBaseConstant.SYS_COMPANY_CODE_TABLE)) {
			returnValue = getSessionUserName().getCurrentDepart().getOrgCode().substring(0, Integer.valueOf(getOrgCodeLengthType()));
		}
		// 替换为系统用户登录所使用的机构编码
	...
	private static String getSessionData(String key) {
		...
		// ---author:jg_xugj----end-----date:20151226--------for：修改bug
		// 1、key.contains("${") 应改为 key.contains("#{") 2、StringUtil.isEmpty(key)
		// 判断 不为空
