...
public class CKFinderConfig extends Configuration {
	...
	protected Configuration createConfigurationInstance() {
		...
		try {
			...
			this.baseURL = FileUtils.path(Servlets.getRequest().getContextPath() + Global.USERFILES_BASE_URL + user.getId() + "/");
			this.baseDir = FileUtils.path(Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL + user.getId() + "/");
		} catch (Exception e) {
	...
	public boolean checkAuthentication(final HttpServletRequest request) {
		return UserCacheUtils.getCurrentUser() != null;
	}
