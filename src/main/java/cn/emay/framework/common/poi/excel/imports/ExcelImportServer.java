 ...
 */
package cn.emay.framework.common.poi.excel.imports;
...
import cn.emay.framework.common.poi.excel.entity.params.ExcelCollectionParams;
import cn.emay.framework.common.poi.excel.entity.params.ExcelImportEntity;
import cn.emay.framework.common.poi.excel.entity.params.ImportParams;
import cn.emay.framework.common.poi.excel.entity.result.ExcelImportResult;
import cn.emay.framework.common.poi.excel.entity.result.ExcelVerifyHanlderResult;
import cn.emay.framework.common.poi.excel.imports.base.ImportBaseService;
import cn.emay.framework.common.poi.exception.excel.ExcelImportException;
import cn.emay.framework.common.poi.exception.excel.enums.ExcelImportEnum;
import cn.emay.framework.common.poi.handler.inter.IExcelModel;
import cn.emay.framework.common.poi.util.PoiPublicUtil;
import cn.emay.framework.common.poi.util.PoiReflectorUtil;
import cn.emay.framework.common.poi.util.PoiValidationUtil;
...
public class ExcelImportServer extends ImportBaseService {
	...
	private void addListContinue(Object object, ExcelCollectionParams param, Row row, Map<Integer, String> titlemap, String targetId, Map<String, PictureData> pictures, ImportParams params) throws Exception {
		Collection collection = (Collection) PoiReflectorUtil.fromCache(object.getClass()).getValue(object, param.getExcelName());
	...
	private <T> List<T> importExcel(Collection<T> result, Sheet sheet, Class<?> pojoClass, ImportParams params, Map<String, PictureData> pictures) throws Exception {
		...
		while (rows.hasNext() && (row == null || sheet.getLastRowNum() - row.getRowNum() > params.getLastOfInvalidRow())) {
			...
			if (params.getKeyIndex() != null && (row.getCell(params.getKeyIndex()) == null || StringUtils.isEmpty(getKeyValue(row.getCell(params.getKeyIndex())))) && object != null) {
	...
	private boolean verifyingDataValidity(Object object, Row row, ImportParams params, Class<?> pojoClass) {
		...
		if (params.getVerifyHanlder() != null) {
			...
			if (!result.isSuccess()) {
				if (cell == null)
					cell = row.createCell(row.getLastCellNum());
				cell.setCellValue((StringUtils.isNotBlank(cell.getStringCellValue()) ? cell.getStringCellValue() + "," : "") + result.getMsg());
				if (object instanceof IExcelModel) {
					IExcelModel model = (IExcelModel) object;
					model.setErrorMsg((StringUtils.isNotBlank(model.getErrorMsg()) ? model.getErrorMsg() + "," : "") + result.getMsg());
				} else {
					isAdd = false;
				}
				verfiyFail = true;
	...
	private void saveImage(Object object, String picId, Map<String, ExcelImportEntity> excelParams, String titleString, Map<String, PictureData> pictures, ImportParams params) throws Exception {
		...
		if (excelParams.get(titleString).getExportImageType() == 1) {
