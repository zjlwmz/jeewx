 ...
 */
package cn.emay.framework.common.poi.pdf.export;
...
import cn.emay.framework.common.poi.cache.ImageCache;
...
import cn.emay.framework.common.poi.excel.export.base.ExportBase;
import cn.emay.framework.common.poi.pdf.entity.PdfExportParams;
import cn.emay.framework.common.poi.pdf.styler.IPdfExportStyler;
import cn.emay.framework.common.poi.pdf.styler.PdfExportStylerDefaultImpl;
 ...
 * 
 * @author JueYue 2015年10月6日 下午8:21:08
 */
public class PdfExportServer extends ExportBase {
	...
	private void createCells(PdfPTable table, Object t, List<ExcelExportEntity> excelParams, int rowHeight) throws Exception {
		...
		for (int k = 0, paramSize = excelParams.size(); k < paramSize; k++) {
			...
			if (entity.getList() != null) {
				...
				if (entity.getType() == 1) {
					createStringCell(table, value == null ? "" : value.toString(), entity, rowHeight, 1, maxHeight);
				} else {
					createImageCell(table, value == null ? "" : value.toString(), entity, rowHeight, 1, maxHeight);
				}
			}
		}
	}
		...
		for (int i = 0, exportFieldTitleSize = excelParams.size(); i < exportFieldTitleSize; i++) {
			...
			if (entity.getList() != null) {
				if (StringUtils.isNotBlank(entity.getName())) {
					createStringCell(table, entity.getName(), entity, 10, entity.getList().size(), 1);
				}
	...
	private void createHeaderRow(PdfExportParams entity, PdfPTable table, int feildLength) {
		...
		if (entity.getSecondTitle() != null) {
			iCell = new PdfPCell(new Phrase(entity.getSecondTitle(), styler.getFont(null, entity.getSecondTitle())));
