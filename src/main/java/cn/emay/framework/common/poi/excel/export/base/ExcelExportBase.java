 ...
 */
package cn.emay.framework.common.poi.excel.export.base;
...
import cn.emay.framework.common.poi.excel.entity.vo.BaseEntityTypeConstants;
import cn.emay.framework.common.poi.excel.entity.vo.PoiBaseConstants;
import cn.emay.framework.common.poi.excel.export.styler.IExcelExportStyler;
import cn.emay.framework.common.poi.util.PoiMergeCellUtil;
		...
		for (int k = indexKey, paramSize = excelParams.size(); k < paramSize; k++) {
			...
			if (entity.getList() != null) {
				...
				if (entity.getType() == BaseEntityTypeConstants.StringType) {
					...
					createDoubleCell(row, cellNum++, value == null ? "" : value.toString(), index % 2 == 0 ? getStyles(false, entity) : getStyles(true, entity), entity);
					if (entity.isHyperlink()) {
						row.getCell(cellNum - 1).setHyperlink(dataHanlder.getHyperlink(row.getSheet().getWorkbook().getCreationHelper(), t, entity.getName(), value));
					}
				} else {
