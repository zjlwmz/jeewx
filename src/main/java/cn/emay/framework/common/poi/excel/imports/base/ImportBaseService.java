 ...
 */
package cn.emay.framework.common.poi.excel.imports.base;
...
import cn.emay.framework.common.poi.excel.annotation.ExcelCollection;
import cn.emay.framework.common.poi.excel.annotation.ExcelEntity;
 ...
 * 导入基础和,普通方法和Sax共用
 * 
 * @author JueYue 2015年1月9日 下午10:25:53
 */
public class ImportBaseService {
	...
	public void getAllExcelField(String targetId, Field[] fields, Map<String, ExcelImportEntity> excelParams, List<ExcelCollectionParams> excelCollection, Class<?> pojoClass, List<Method> getMethods) throws Exception {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (PoiPublicUtil.isCollection(field.getType())) {
				...
				collection.setExcelName(field.getName());
				Map<String, ExcelImportEntity> temp = new HashMap<String, ExcelImportEntity>();
				ParameterizedType pt = (ParameterizedType) field.getGenericType();
				Class<?> clz = (Class<?>) pt.getActualTypeArguments()[0];
				collection.setType(clz);
				getExcelFieldList(StringUtils.isNotEmpty(excel.id()) ? excel.id() : targetId, PoiPublicUtil.getClassFields(clz), clz, temp, null);
				collection.setExcelParams(temp);
				collection.setExcelName(PoiPublicUtil.getValueByTargetId(field.getAnnotation(ExcelCollection.class).name(), targetId, null));
				...
				getAllExcelField(StringUtils.isNotEmpty(excel.id()) ? excel.id() : targetId, PoiPublicUtil.getClassFields(field.getType()), excelParams, excelCollection, field.getType(), newMethods);
			}
		}
	}
	...
	public void getExcelField(String targetId, Field field, ExcelImportEntity excelEntity, Excel excel, Class<?> pojoClass) throws Exception {
		excelEntity.setName(PoiPublicUtil.getValueByTargetId(excel.exportName(), targetId, null));
	...
	public void getExcelFieldList(String targetId, Field[] fields, Class<?> pojoClass, Map<String, ExcelImportEntity> temp, List<Method> getMethods) throws Exception {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (PoiPublicUtil.isJavaClass(field)) {
				...
				getExcelFieldList(StringUtils.isNotEmpty(excel.id()) ? excel.id() : targetId, PoiPublicUtil.getClassFields(field.getType()), field.getType(), temp, newMethods);
			}
		}
	}
