package cn.emay.framework.common.poi.excel.html;
...
import cn.emay.framework.common.poi.excel.html.helper.CellValueHelper;
import cn.emay.framework.common.poi.excel.html.helper.MergedRegionHelper;
import cn.emay.framework.common.poi.excel.html.helper.StylerHelper;
import cn.emay.framework.common.poi.util.PoiPublicUtil;

/**
 * Excel转换成Html 服务
 * 
 * @author JueYue 2015年5月10日 上午11:41:15
 */
public class ExcelToHtmlServer {
	...
	private void getPictures() {
		if (wb instanceof XSSFWorkbook) {
			pictures = PoiPublicUtil.getSheetPictrues07((XSSFSheet) wb.getSheetAt(sheetNum), (XSSFWorkbook) wb);
		} else {
			pictures = PoiPublicUtil.getSheetPictrues03((HSSFSheet) wb.getSheetAt(sheetNum), (HSSFWorkbook) wb);
		}
	}
			...
			for (int i = firstColumn; i < endColumn; i++) {
				if (mergedRegionHelper.isNeedCreate(rowIndex, i)) {
					...
					if (pictures.containsKey((rowIndex - 1) + "_" + i)) {
						content = "<img src='" + getImageSrc(pictures.get((rowIndex - 1) + "_" + i)) + "' style='max-width:  " + getImageMaxWidth(mergedRegionHelper.getRowAndColSpan(rowIndex, i), i, sheet) + "px;' />";
					}
