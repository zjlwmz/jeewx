...
import cn.emay.framework.tag.vo.easyui.ColumnValue;
import cn.emay.framework.tag.vo.easyui.DataGridColumn;
import cn.emay.framework.tag.vo.easyui.DataGridUrl;
import cn.emay.framework.tag.vo.easyui.OptTypeDirection;
	...
	public void setColumn(String title, String field, Integer width,Integer showLen,String rowspan, 
		...
		if (null!=operationCodes) {
			for (String MyoperationCode : operationCodes) {
				if (oConvertUtils.isEmpty(MyoperationCode))
					break;
				systemService = SpringContextHolder.getApplicationContext().getBean(
							SystemService.class);
		...
		if (!StringUtils.isBlank(dictionary)&&(!popup)) {
			if(dictionary.contains(",")){
				...
				String text = "";
				String value = "";
				List<Type> typeList = ResourceUtil.allTypes.get(dictionary.toLowerCase());
...
//-----author:jg_longjb----end-----date:20150408--------for:读取cookie主题样式 ace界面下table的输出 
	...
	public StringBuffer end() {
		...
		if(hasQueryColum(columnList)){
			...
			sb.append("<script type=\"text/javascript\" src=\"static/Validform/js/Validform_v5.3.1_min_zh-cn.js\"></script>");
			sb.append("<script type=\"text/javascript\" src=\"static/Validform/js/Validform_Datatype_zh-cn.js\"></script>");
			sb.append("<script type=\"text/javascript\" src=\"static/Validform/js/datatype_zh-cn.js\"></script>");
...
//											+ " as text from " + dic[0];
								...
								//	<input type="text" name="order_code"  style="width: 100px"  class="searchbox-inputtext" value="" onClick="inputClick(this,'account','user_msg');" />
									...
									Map<String, List<Type>> typedatas = ResourceUtil.allTypes;
									List<Type> types = typedatas.get(col.getDictionary().toLowerCase());
									sb.append("<select name=\""+col.getField().replaceAll("_","\\.")+"\" WIDTH=\"100\" style=\"width: 104px\"> ");
									sb.append(StringUtil.replaceAll("<option value =\"\" >{0}</option>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.please.select")));
									if (types != null) {
										for (Type type : types) {
	...
	private String extendAttribute(String field) {
		...
		try{
			JSONObject obj =JSON.parseObject(field);
...
//	}
	...
	public String getNoAuthOperButton(){
		...
		//cn.emay.framework.core.util.LogUtil.info("----getNoAuthOperButton-------"+sb.toString());
		return sb.toString(); 
	}
	...
	public StringBuffer aceStyleTable() {
		...
		width = (width == null) ? "auto" : width;
		height = (height == null) ? "auto" : height;
//		sb.append("<link rel=\"stylesheet\" href=\"static/easyui/themes/metro/main.css\" />");
		sb.append("<script type=\"text/javascript\">");
		sb.append("$(function(){  storage=$.localStorage;if(!storage)storage=$.cookieStorage;");
		sb.append(this.getNoAuthOperButton());
		if (treegrid) {
...
//		appendLine(sb,"				{'fieldId':'office_Phone','fieldName':'办公电话'},");
		...
		appendLine(sb,"					[  ");
		Map<String, List<Type>> typedatas = ResourceUtil.allTypes;
		List<Type> types = typedatas.get("rulecon");
		if (types != null) {
		...
		//已在baseTag中引入
//		appendLine(sb,"<script type=\"text/javascript\" src=\"static/jquery/jquery.cookie.js\" ></script>");
//		appendLine(sb,"<script type=\"text/javascript\" src=\"static/jquery-plugs/storage/jquery.storageapi.min.js\" ></script>"); 
