package cn.emay.framework.core.extend.hqlsearch;
...
import cn.emay.framework.core.annotation.query.QueryTimeFormat;
import cn.emay.framework.core.common.hibernate.qbc.CriteriaQuery;
import cn.emay.framework.core.extend.QueryCondition;
...
public class HqlGenerateUtil {
	...
	public static void installHql(CriteriaQuery cq, Object searchObj, Map<String, String[]> parameterMap) {
		...
		try {
			String json = null;
			if (null!=cq.getDataGrid() && StringUtil.isNotEmpty(cq.getDataGrid().getSqlbuilder())) {
			...
			if (StringUtil.isNotEmpty(json)) {
				List<QueryCondition> list = JSON.parseArray(json, QueryCondition.class);
	 ...
	 * @param alias
	 */
	private static void installHqlJoinAlias(CriteriaQuery cq, Object searchObj, Map<String, DataRule> ruleMap, Map<String, String[]> parameterMap, String alias) {
		PropertyDescriptor origDescriptors[] = PropertyUtils.getPropertyDescriptors(searchObj);
		String aliasName, name, type;
		for (int i = 0; i < origDescriptors.length; i++) {
			aliasName = (alias.equals("") ? "" : alias + ".") + origDescriptors[i].getName();
			name = origDescriptors[i].getName();
			if(null!=origDescriptors[i].getPropertyType()){
	 ...
	 * @return
	 */
	private static boolean isHaveRuleData(Map<String, DataRule> ruleMap, String aliasName) {
	...
	private static void addRuleToCriteria(DataRule tsDataRule, String aliasName, Class propertyType, CriteriaQuery cq) {
	...
	private static String converRuleValue(String ruleValue) {
		...
		// ---author:jg_xugj----end-----date:20151226--------for：#814
		// 【数据权限】扩展支持写表达式，通过session取值
	...
	private static Map<String, DataRule> getRuleMap() {
		...
		List<DataRule> list = JeecgDataAutorUtils.loadDataSearchConditonSQL(); // (List<TSDataRule>)
																				// ContextHolderUtils
	...
	/**
	 * @author ljb 根据对象拼装sql TODO 结合DataRule
	 * @param list
	 * @param tab格式化
	 * @return
	 */
	public static String getSql(List<QueryCondition> list, String tab, Class claszz) {
		...
		for (QueryCondition c : list) {
			...
			if (c.getChildren() != null) {
				...
				List list1 = c.getChildren();// JSONHelper.toList(,
	...
	/**
	 * 根据字段名称,获取字段的类型字符串 return: java.lang.Integer
	 */
	...
	public static CriteriaQuery getDataAuthorConditionHql(CriteriaQuery cq, Object searchObj) {
		Map<String, DataRule> ruleMap = getRuleMap();
		PropertyDescriptor origDescriptors[] = PropertyUtils.getPropertyDescriptors(searchObj);
		String aliasName, name, type;
		for (int i = 0; i < origDescriptors.length; i++) {
