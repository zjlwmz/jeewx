package cn.emay.framework.core.common.dao.jdbc;
...
//			sql =MessageFormat.format(MYSQL_SQL, sql,beginNum,rows);
			...
			if(ResourceUtil.getJdbcUrl().indexOf(DATABSE_TYPE_ORACLE)!=-1) {
				sql =MessageFormat.format(ORACLE_SQL, sql,beginNum,rows);
			} else if(ResourceUtil.getJdbcUrl().indexOf(DATABSE_TYPE_SQLSERVER)!=-1) {
				sqlParam[0] = sql.substring(getAfterSelectInsertPoint(sql));
				sql =MessageFormat.format(SQLSERVER_SQL, sql,beginNum,rows);
			}
		}
		return sql;
	}
