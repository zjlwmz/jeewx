package cn.emay.modules.sms.web;
...
import cn.emay.modules.sms.utils.TuiSongMsgUtil;
...
//	private static final Logger logger = Logger.getLogger(TSSmsTemplateSqlController.class);
	...
	public void datagrid(TSSmsTemplateSqlEntity tSSmsTemplateSql,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		//查询条件组装器
		cn.emay.framework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tSSmsTemplateSql, request.getParameterMap());
		try{
		//自定义追加查询条件
		}catch (Exception e) {
