...
import cn.emay.framework.core.common.model.json.Highchart;
import cn.emay.framework.tag.core.easyui.TagUtil;
import cn.emay.modules.sys.entity.Log;
import cn.emay.modules.sys.service.LogService;
...
//        add-begin--Author:zhangguoming  Date:20140427 for：添加查询条件  操作时间
        ...
        if(operatetime_begin != null) {
            ...
            beginValue =new Timestamp(DateUtils.parseDate(operatetime_begin).getTime());
        ...
        if(operatetime_end != null) {
            ...
            endValue =new Timestamp(DateUtils.parseDate(operatetime_end).getTime());
...
//        add-end--Author:zhangguoming  Date:20140427 for：添加查询条件  操作时间
	...
	public ModelAndView logDetail(Log tsLog,HttpServletRequest request){
		if (StringUtil.isNotEmpty(tsLog.getId())) {
			tsLog = logService.getEntity(Log.class, tsLog.getId());
	...
	public List<Highchart> getBroswerBar(HttpServletRequest request,String reportType, HttpServletResponse response) {
		...
		List<Object> userBroswerList = systemService.findByQueryString(sb.toString());
		Long count = systemService.getCountForJdbc("SELECT COUNT(1) FROM sys_log WHERE 1=1");
