...
<html>
 ...
 <body>
	<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="div" action="tSTeamPersonController.do?doUpdate" tiptype="1">
		...
		<fieldset class="step">
			<div class="form">
				...
				<input class="inputxt" id="name" name="name" value="${tSTeamPersonPage.name}" datatype="*">
				<span class="Validform_checktip"></span>
			</div>
			
			<div class="form">
				<label class="Validform_label">头像:</label>
				<input type="hidden" id="imgSrc" name="imgSrc" value='${tSTeamPersonPage.imgSrc}'/>
				<c:if test="${empty tSTeamPersonPage.imgSrc}">
					<a   target="_blank" id="imgSrc_href">暂时未上传文件</a>
				</c:if>
				<c:if test="${!empty tSTeamPersonPage.imgSrc}">
					<a href="${tSTeamPersonPage.imgSrc}"  target="_blank" id="imgSrc_href" href="${tSTeamPersonPage.imgSrc}">下载</a>
				</c:if>
			   <input class="ui-button" type="button" value="上传附件"
							onclick="commonUpload(imgSrcCallback)"/>
	     			<script type="text/javascript">
			...
			<div class="form">
				...
				<textarea style="width:600px;" class="inputxt" rows="6" id="introduction" name="introduction" datatype="*">${tSTeamPersonPage.introduction}</textarea>
				<span class="Validform_checktip"></span>
			</div>
			
			<div class="form">
				<label class="Validform_label">加入时间:</label>
				<input id="jionDate" name="jionDate" type="text" style="width: 150px" class="Wdate" onClick="WdatePicker()" value="<fmt:formatDate value='${tSTeamPersonPage.jionDate}' type="date" pattern="yyyy-MM-dd"/>">
				<span class="Validform_checktip"></span>
			</div>
            <div class="form">
                <label class="Validform_label">是否参与:</label>
                <select id="isJoin" name="isJoin" style="width: 150px" >
                    <option value="1" ${tSTeamPersonPage.isJoin eq '1'? 'selected':'' }>参与</option>
                    <option value="0" ${tSTeamPersonPage.isJoin eq '0'? 'selected':'' } >不参与</option>
                </select>
                <span class="Validform_checktip"></span>
            </div>
		</fieldset>
