...
<t:formvalid formid="formobj" layout="div" dialog="true" refresh="true" action="functionController.do?saveFunction">
	...
	<div class="form" id="pfun">
        ...
        <input id="cc" <c:if test="${function.parentFunction.functionLevel eq 0}"> value="${function.parentFunction.id}"</c:if>
		<c:if test="${function.parentFunction.functionLevel > 0}"> value="${function.parentFunction.functionName}"</c:if>>
        <input id="functionId" name="parentFunction.id" style="display: none;" value="${function.parentFunction.id}">
    </div>
    ...
    <div class="form">
        ...
        <select name="icon.id">
            <c:forEach items="${iconlist}" var="icon">
                <option value="${icon.id}" <c:if test="${icon.id==function.icon.id || (function.id eq null && icon.iconClas eq 'default') }">selected="selected"</c:if>>
    ...
    <div class="form">
        ...
        <select name="iconDesk.id">
            <c:forEach items="${iconDeskList}" var="icon">
                <option value="${icon.id}" <c:if test="${icon.id==function.iconDesk.id || (function.id eq null && icon.iconClas eq 'System Folder') }">selected="selected"</c:if>>
                    <t:mutiLang langKey="${icon.iconName}"/>
                </option>
            </c:forEach>
        </select>
