...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  //聊天内容列表模版
  var elemChatMain1 = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
               	      ...
               	      ,'<li {{ d.mine ? "class=layim-chat-mine" : "" }}>'
               	      		,'<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>'
               		      		...
               		      		,'{{# } else if(d.msgType =="image"){ }}'
               		      			,'<div class="layim-chat-text layim-chat-image">{{ layui.data.content(d.content) }}</div>'
               		      		,'{{# } else if(d.msgType =="voice"){ }}'
               		      			,'<div class="layim-chat-text voice">'
		               		      		//,'<audio controls="controls"><source src="{{ d.content }}" /> </audio>
		               				 	,'<p class="layui-layim-weixinAudio" sound="{{ d.content }}" style="min-width:110px;width:{{d.width}}">'
	               					 		,'<audio src="{{ d.content }}" class="media" width="1" height="1" preload></audio>'
	               						 	,'<span class="db audio_area">'
		               						 	,'<span class="audio_wrp db">'
			               						 	...
			               						 	,'</span>'
			               						    ,'<span class="audio_length tips_global">{{ d.mediaTime }}</span>'
		               				   ...
		               				   ,'</p>'
		               		 	,'</div>'
		               		 	,'{{# } else if(d.msgType=="shortvideo" ||  d.msgType=="video"){ }}'
		               			 	,'<div class="layim-chat-text video">'
			               			 	,'<div class="layui-layim-willesPlay" style="width:350px;">'
			               			 		,'<div class="playContent">'
	                     						,'<video width="100%" height="100%">'
	                     							,'<source src="{{d.content}}" type="video/mp4" />'
			               						,'</video>'
			               						,'<div class="playTip"><i class="layui-icon" style="font-size: 25px;">&#xe623;</i></div>'
			               					,'</div>'
			               					,'<div class="playControll" style="display:none;">'
			               						,'<div class="playPause playIcon"></div>'
			               						,'<div class="timebar">'
			               							,'<span class="currentTime">0:00:00</span>'
			               							,'<div class="layui-progress progress">'
			               								,'<div class="layui-progress-bar progress-bar progress-bar-danger progress-bar-striped" role="progressbar" aria-valuemin="0" aria-valuemax="100" style="width: 0%"></div>'
			               								,'</div>'
			               							,'<span class="duration">0:00:00</span>'
			               						  ,'</div>'
			               						 ,'<div class="otherControl">'
			               						 		,'<span><i class="volume layui-icon"  style="font-size:30px;">&#xe645;</i></span>'
			               						 		,'<div class="volumeBar">'
			               						 			,'<div class="volumewrap">'
			               						 				,'<div class="layui-progress">'
			               						 					,'<div class="layui-progress-bar progress-bar progress-bar-danger" lay-percent="10%" style="height: 100%;"></div>'
			               						 					,'</div>'
			               						 				,'</div>'
			               						 			,'</div>'
			               						 		,'</div>'
  ...
  layui.data.content = function(content){
    ...
    var html = function(end){
      return new RegExp('\\n*\\['+ (end||'') +'(pre|div|p|table|thead|th|tbody|tr|td|ul|li|ol|li|dl|dt|dd|h2|h3|h4|h5|img)([\\s\\S]*?)\\]\\n*', 'g');
    };
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
    ...
    var index = chatIndex = layer.open({
      ...
      ,success: function(layero){
        ...
        //语音播放
        layero.on('click', '.layui-layim-weixinAudio', function(){
        	layui.audio($(this),{autoplay:true});
        });
        //视频播放
        layero.on('click', '.layui-layim-willesPlay', function(){
        	layui.vedio($(this),{autoplay:true});
        });
  ...
  //微信表情库
  var faces = function(){
      var alt=["[微笑]","[撇嘴]","[色]","[发呆]","[得意]","[流泪]","[害羞]","[闭嘴]","[睡]","[大哭]",
