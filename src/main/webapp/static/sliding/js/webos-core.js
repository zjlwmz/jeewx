	...
	return me ={
		 ...
		 bindTheme:function(){
			 ...
			 $("a",themsSetting).live("click",function(){
					...
					var h = $(window).height();
					var w = $(window).width();
					$("#zoomWallpaper").attr("src","static/sliding/images/bg/"+src+".jpg").width(w).height(h);

					$.cookie("myskin","static/sliding/images/bg/"+src+".jpg",cookieParam);
...
//	dataFlush();
	...
	var num =totalnum+1 ,////替换掉一级菜单的总数量
		defaultnum=1,
		title = "请登录",
		usericon = "static/sliding/images/jeecg.png";
...
Task = $.Class({
	...
	create:function(op)	{
		...
		$("<img src='static/sliding/icon/"+op.icon+"'/><div class='taskItemIconState'></div>").appendTo(taskItemIcon);
...
//			src:'static/sliding/icon/'+this.app.icon,
			src:this.app.icon,
...
//			src:'static/sliding/icon/'+this.sApp.icon,
			src:this.sApp.icon,
